...
import (
	...
	"strings"
	"time"
	"weather_mgr/cootek/account/TokenService"
...
func (this tokenService) testToken(c *gin.Context, authToken string) (ant *account.Account, ok bool) {
	...
	data := map[string]string{
		...
		"cn01:0d89978e-35d7-4d5e-bb7c-4cd47722d002": `{"user_id":26,"account_type":1,"activation_info":{"app_name":"com.pedometer.money.cn","app_version":"1024","os_name": "android"}}`,
		"180cbbe1-2313-45bc-b89d-35b166524cf9":      `{"user_id":27,"account_type":1,"activation_info":{"app_name":"com.androidhealth.steps.money","app_version":"1024","os_name": "iOS"}}`,
	}
	s, ok := data[authToken]
	if !ok {
		return
...
func (this tokenService) SetUserInfo(c *gin.Context, ant *account.Account) {
	ctx := c.Request.Context()
	ctx = context.WithValue(ctx, "account", ant)
	ctx = context.WithValue(ctx, "app_name", ant.ActivationInfo.GetAppName())
	ctx = context.WithValue(ctx, "version", ant.ActivationInfo.GetAppVersion())
	ctx = context.WithValue(ctx, "uid", ant.GetUserId())
	//xzap.Debug("set", zap.Any("uid", ant.GetUserId()))
	ctx = context.WithValue(ctx, "os", ant.ActivationInfo.GetOsName())
	ctx = context.WithValue(ctx, "activate_timestamp", int64(ant.ActivationInfo.GetActivateTimestamp()))
